...
- **Title label.** `TipNotificationFactory` appends a `· Advanced` label to the notification title — `Vim Coach · Advanced` — when `tip.advanced` is true; normal tips keep the plain `Vim Coach` title. Only opted-in users ever draw an advanced tip, so the label needs no in-app legend.
